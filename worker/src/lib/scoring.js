...
export const SCORING_VERSION = "v3-alpha-1";
...
function maxEventDate(events) {
  return events
    .map((event) => event.event_date || event.observed_at)
    .filter(Boolean)
    .sort()
    .at(-1) || null;
}
...
function latestOf(events, types) {
  return eventsOf(events, types)
    .slice()
    .sort((a, b) =>
      String(b.event_date || b.observed_at || "").localeCompare(
        String(a.event_date || a.observed_at || "")
      )
    )[0] || null;
}

function determinePhase(events, activeJobs, hasPhase3Evidence) {
  const fieldHiring = eventsOf(events, FIELD_EVENTS).length + activeJobs.field;
  const leadershipHiring = eventsOf(events, LEADERSHIP_EVENTS).length + activeJobs.leadership;
  ...
  const recentApproval = latestOf(events, ["FDA_APPROVED"]);
  if (recentApproval && daysSince(recentApproval.event_date) <= 180) return 5;

  if (
    hasEvent(events, "INTERNAL_US_LAUNCH_PLANNED") ||
    hasEvent(events, "LAUNCH_TIMING_DISCLOSED") ||
    hasEvent(events, "US_RIGHTS_INTERNAL") ||
    hasEvent(events, "US_RIGHTS_ACQUIRED") ||
    hasPhase3Evidence
  ) {
    return 1;
  }
...
function regulatoryScore(events, hasPhase3Evidence) {
  const approval = latestOf(events, ["FDA_APPROVED"]);
  if (approval) {
  ...
  if (hasEvent(events, "PDUFA_ASSIGNED")) return 18;
  if (hasEvent(events, "ADCOM_SCHEDULED")) return 17;
  if (hasEvent(events, "APPLICATION_ACCEPTED")) return 15;
  if (hasEvent(events, "NDA_BLA_SUBMITTED")) return 13;
  if (hasEvent(events, "NDA_BLA_PLANNED")) return 10;
  if (hasEvent(events, "PIVOTAL_POSITIVE")) return 8;
  if (hasPhase3Evidence) return 7;
  return 0;
}

function ownershipScore(events) {
  const partnered = hasEvent(events, "US_RIGHTS_PARTNERED");
  const coCommercial = hasEvent(events, "CO_COMMERCIALIZATION");
  const acquired = hasEvent(events, "US_RIGHTS_ACQUIRED");
  const internal = hasEvent(events, "US_RIGHTS_INTERNAL");
  const internalLaunch = hasEvent(events, "INTERNAL_US_LAUNCH_PLANNED");

  if (partnered) return 2;
  if (coCommercial) return 12;
  if (acquired) return internalLaunch ? 20 : 18;
  if (internal) return internalLaunch ? 20 : 17;
  return 7;
}

function orgBuildScore(events, phase) {
  if (phase >= 4) return 19;
  if (hasEvent(events, "RBD_JOB_POSTED") || hasEvent(events, "SALES_MANAGER_JOB_POSTED")) return 18;
  ...
  if (hasEvent(events, "INTERNAL_US_LAUNCH_PLANNED")) return 4;
  return 1;
}

function hiringScore(events, jobs) {
  const eventLeadership = eventsOf(events, LEADERSHIP_EVENTS).length;
  const eventField = eventsOf(events, FIELD_EVENTS).length;
  const activeCommercial = jobs.activeCommercial;
  const new30 = jobs.new30;
  const new90 = jobs.new90;
  const regions = jobs.regions;

  let score = 0;

  if (eventLeadership > 0 || jobs.leadership > 0) score = Math.max(score, 10);
  if (eventLeadership >= 2 || jobs.leadership >= 2) score = Math.max(score, 13);
  if (eventField > 0 || jobs.field > 0) score = Math.max(score, 15);
  if (eventField >= 2 || jobs.field >= 2 || regions >= 2) score = Math.max(score, 18);
...
function coverageGroup(sourceKey) {
  ...
  if (sourceKey === "openfda_drugsfda" || sourceKey === "fda_adcom" || sourceKey === "fda_tracker") return "regulatory";
  if (sourceKey === "company_ir") return "company";
  if (sourceKey === "company_careers" || sourceKey === "greenhouse" || sourceKey === "lever" || sourceKey === "ashby") return "hiring";
...
function evidenceConfidence(events, evidence, checks) {
  ...
  let authority = 10;
  if (primary > 0) authority = 35;
  else if (tierA > 0) authority = 28;
  else if (tierB > 0) authority = 20;

  let corroboration = 0;
  if (maxSources >= 3) corroboration = 20;
  else if (maxSources >= 2) corroboration = 16;
  else if (evidence.length >= 2) corroboration = 10;
  else if (evidence.length === 1) corroboration = 5;
  ...
  let freshness = 2;
  if (age <= 90) freshness = 10;
  else if (age <= 180) freshness = 8;
  else if (age <= 365) freshness = 6;
  else if (age <= 730) freshness = 4;

  const rightsConflict =
    hasEvent(events, "US_RIGHTS_PARTNERED") &&
    (hasEvent(events, "US_RIGHTS_INTERNAL") || hasEvent(events, "US_RIGHTS_ACQUIRED"));
...
function unresolvedRegulatorySetback(events) {
  ...
  const recovery = latestOf(events, [
    "PIVOTAL_POSITIVE",
    "NDA_BLA_SUBMITTED",
    "APPLICATION_ACCEPTED",
    "PDUFA_ASSIGNED",
  ...
  return String(recovery.event_date || recovery.observed_at || "") <=
    String(setback.event_date || setback.observed_at || "");
}

function applyCaps(score, events, jobs) {
  let capped = score;
  const caps = [];

  const internalHiring =
    eventsOf(events, HIRING_EVENTS).length > 0 ||
    jobs.activeCommercial > 0;
  ...
  const expansionCatalyst =
    internalHiring ||
    hasEvent(events, "ASSET_ACQUIRED") ||
    hasEvent(events, "INTERNAL_US_LAUNCH_PLANNED");
...
function recommendation({ score, confidence, coverage, phase, events, jobs, rightsConflict }) {
  const hiringSignal = eventsOf(events, HIRING_EVENTS).length > 0 || jobs.activeCommercial > 0;
  const strongCatalyst = events.some((event) => STRONG_REGULATORY_EVENTS.has(event.event_type));
  ...
  if (
    ...
    coverage >= 50 &&
    !criticalConflict &&
    (hiringSignal || strongCatalyst)
  ) {
    return "CONTACT NOW";
  }

  if (score >= 60 && (confidence < 70 || coverage < 50 || criticalConflict)) {
...
function rationale({ events, phase, caps, hasPhase3Evidence, jobs }) {
  const points = [];
  if (hasPhase3Evidence) points.push("Phase III clinical evidence present");
  if (hasEvent(events, "FDA_APPROVED")) points.push("recent/known FDA approval");
  ...
  if (hasEvent(events, "INTERNAL_US_LAUNCH_PLANNED")) points.push("internal U.S. launch intent supported");
  if (eventsOf(events, LEADERSHIP_EVENTS).length) points.push("sales leadership build detected");
  if (eventsOf(events, FIELD_EVENTS).length || jobs.field) points.push("field-sales hiring detected");
  ...
  if (caps.length) points.push(`score cap: ${caps.join("; ")}`);
...
async function targetFacts(DB, companyId, assetId) {
  const [eventsResult, evidenceResult, checksResult, jobsResult, trialResult] = await Promise.all([
    DB.prepare(
      `SELECT id, event_type, event_date, observed_at, status, value_text,
              value_number, value_json, evidence_confidence,
    ...
    DB.prepare(
      `SELECT COUNT(*) AS n
        ...
        WHERE oi.company_id = ?
          ...
          AND (
            json_extract(o.payload_json, '$.phase') LIKE '%PHASE3%'
            OR json_extract(o.payload_json, '$.phases') LIKE '%PHASE3%'
          )`
...
async function upsertOpportunity(DB, target, score) {
  const existing = await DB.prepare(
    `SELECT * FROM opportunities WHERE company_id = ? AND asset_id = ? LIMIT 1`
  ...
  const materiallyChanged =
    !existing ||
    Number(existing.hiring_phase) !== score.phase ||
    Number(existing.opportunity_score) !== score.total ||
    Number(existing.evidence_confidence) !== score.confidence ||
  ...
  if (existing && existing.recommended_action !== score.action) {
    await DB.prepare(
      ...
      .bind(
        ...
        score.action,
        score.action === "CONTACT NOW" ? "high" : "info",
        scoredAt
      )
      .run();
  }
...
export async function scoreOpportunities(DB, { limit = 500 } = {}) {
  ...
  for (const target of targets.results || []) {
    ...
    const phase = determinePhase(facts.events, facts.jobs, facts.hasPhase3Evidence);
    const regulatory = regulatoryScore(facts.events, facts.hasPhase3Evidence);
    const ownership = ownershipScore(facts.events);
    const orgBuild = orgBuildScore(facts.events, phase);
    const hiring = hiringScore(facts.events, facts.jobs);
    const timing = TIMING_POINTS[phase] ?? 0;
    const execution = executionScore(facts.events);

    const rawTotal = clamp(
      regulatory + ownership + orgBuild + hiring + timing + execution,
      0,
      100
    );

    const capped = applyCaps(rawTotal, facts.events, facts.jobs);
    ...
    const score = {
      ...
      rationale: rationale({
        ...
        phase,
        caps: capped.caps,
        hasPhase3Evidence: facts.hasPhase3Evidence,
        jobs: facts.jobs,
      }),
    };

    const didChange = await upsertOpportunity(DB, target, score);
...
export async function listOpportunities(DB, { limit = 200 } = {}) {
  ...
  const result = await DB.prepare(
    `SELECT
       ...
       o.hiring_phase,
       o.hiring_phase_label,
       o.regulatory_score,
       o.commercial_ownership_score,
       o.org_build_score,
       o.hiring_score,
       o.timing_score,
       o.execution_capacity_score,
       o.opportunity_score,
       o.evidence_confidence,
       o.source_coverage,
       o.recommended_action,
       o.next_expected_hiring_event,
       o.rationale,
       o.scored_at,
       o.scoring_version,
