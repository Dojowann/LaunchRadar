    ...
    targets: {
      ...
      items: {
        ...
        properties: {
          ...
          provider: {
            type: "string",
            enum: ["greenhouse", "lever", "ashby", "none"],
          },
...
function parseAtsTarget(provider, rawUrl) {
  ...
  if (provider === "greenhouse") {
    ...
    if (!key) return null;
    return { sourceKey: "greenhouse", targetKey: key, targetUrl: cleaned };
  }
  ...
  if (provider === "lever") {
    ...
    let key = null;
    if (host === "jobs.lever.co") key = parts[0] || null;
    else {
  ...
  if (provider === "ashby") {
    ...
    let key = null;
    if (host === "jobs.ashbyhq.com") key = parts[0] || null;
    else {
      const boardIndex = parts.indexOf("job-board");
      if (boardIndex >= 0) key = parts[boardIndex + 1] || null;
    }
    if (!key) return null;
    return { sourceKey: "ashby", targetKey: key, targetUrl: cleaned };
  }
...
async function existingTargets(DB, companyIds) {
  ...
  const result = await DB.prepare(
    `SELECT st.id, st.company_id, st.source_key, st.target_key, st.target_url,
            st.metadata_json, c.canonical_name AS company_name
       FROM source_targets st
       JOIN companies c ON c.id = st.company_id
      WHERE st.enabled = 1
        AND st.source_key IN ('greenhouse','lever','ashby')
...
async function discoverMissingTargets(env, companies) {
      ...
      input: [
        {
          role: "system",
          content:
            "Find the current official careers job board for each company. Only report Greenhouse, Lever, or Ashby boards. Prefer a link reached from the company's official careers site. Never guess a board slug. Return provider=none when no current board on those three systems can be verified. board_url must be a real URL found during web research.",
        },
        {
          role: "user",
  ...
  if (!response.ok) {
    throw new Error(payload?.error?.message || `OpenAI ATS discovery returned ${response.status}.`);
  }
...
async function upsertTarget(DB, target) {
  const existing = await DB.prepare(
    `SELECT id
       FROM source_targets
      WHERE company_id = ? AND source_key = ? AND target_key = ?
      LIMIT 1`
  )
  ...
  if (existing?.id) {
    await DB.prepare(
      `UPDATE source_targets
          SET target_url = ?, enabled = 1, metadata_json = ?, updated_at = ?
        WHERE id = ?`
    )
  ...
  await DB.prepare(
    `INSERT INTO source_targets
      (id, company_id, source_key, target_key, target_url, enabled, metadata_json, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?)`
  )
    .bind(
      id,
...
async function writeCompanyCareersCheck(DB, scanRunId, companyId, recordsFound, errorText = null) {
  await DB.prepare(
    `INSERT INTO source_checks
      (id, source_key, scan_run_id, company_id, asset_id, status, started_at,
       completed_at, records_found, query_json, error_text, latest_source_date)
    ...
    .bind(
      ...
      isoNow(),
      recordsFound,
      JSON.stringify({ purpose: "ATS discovery" }),
      errorText
    )
    .run();
}
...
async function writeProviderCheck(DB, scanRunId, target, recordsFound, latestSourceDate, errorText = null) {
  await DB.prepare(
    ...
    .bind(
      ...
      isoNow(),
      recordsFound,
      JSON.stringify({ targetKey: target.targetKey, targetUrl: target.targetUrl }),
      errorText,
...
async function collectTarget(target) {
  ...
  if (target.sourceKey === "lever") {
    ...
    return collectLever({
      ...
      companyName: target.companyName,
      region: metadata.region || target.region || "global",
    });
  }
...
function eventTypeForJob(title, roleFamily) {
  ...
  if (roleFamily === "sales_leadership") {
    if (
      /regional business director|regional sales director|area business director|area sales director/.test(t)
    ) {
  ...
  if (roleFamily === "field_sales") {
    if (/key account manager|strategic account manager|\bkam\b/.test(t)) return "KAM_JOB_POSTED";
...
async function upsertJobInventory(DB, target, observation) {
  ...
  const existing = await DB.prepare(
    `SELECT id, first_seen_at
       FROM job_postings
      WHERE company_id = ? AND source_key = ? AND external_job_id = ?
      LIMIT 1`
  )
...
async function evidenceAndEventForJob(DB, target, job) {
  if (!job?.role?.commercial) return { eventCreated: false, evidenceCreated: false };
...
async function scanTarget(DB, scanRunId, target) {
  ...
  await DB.prepare(
    `UPDATE job_postings
      ...
      WHERE company_id = ? AND source_key = ? AND status = 'active'`
  )
...
export async function scanHiringBatch(DB, env, { batchSize = 5 } = {}) {
  ...
  await DB.prepare(
    ...
    .bind(
      scanRunId,
      startedAt,
      JSON.stringify({ batchSize, companyIds: companies.map((x) => x.id) }),
      companies.length
    )
    .run();
  ...
  try {
    ...
    for (const target of existing) {
      if (!existingByCompany.has(target.company_id)) existingByCompany.set(target.company_id, []);
      existingByCompany.get(target.company_id).push(target);
    }

    const missing = companies.filter((company) => !existingByCompany.has(company.id));
    ...
    for (const target of discovered) {
      if (!discoveredByCompany.has(target.companyId)) discoveredByCompany.set(target.companyId, []);
    ...
    await DB.prepare(
      `UPDATE scan_runs
          SET status = 'complete', completed_at = ?, sources_completed = ?,
              observations_found = ?
        WHERE id = ?`
    )
